...
export default function BroadcastDetailPage() {
  ...
  const tStatuses = useTranslations('broadcasts.statuses');
  const tRecipientStatuses = useTranslations('broadcasts.recipientStatuses');
  ...
  function handleExport() {
    if (!broadcast) return;
    const header = [
      t('exportHeaders.contact'),
      t('exportHeaders.phone'),
      t('exportHeaders.status'),
      t('exportHeaders.sentAt'),
      t('exportHeaders.deliveredAt'),
      t('exportHeaders.readAt'),
      t('exportHeaders.repliedAt'),
      t('exportHeaders.error'),
    ];
  ...
  async function handleDelete() {
    ...
    setDeleting(false);
    if (delErr) {
      toast.error(t('deleteFailed', { error: delErr.message }));
      return;
    }
  ...
  const funnelSteps: FunnelStep[] = [
    { label: t('funnelSteps.sent'), value: broadcast.sent_count, color: 'bg-primary' },
    { label: t('funnelSteps.delivered'), value: broadcast.delivered_count, color: 'bg-teal-500' },
    { label: t('funnelSteps.read'), value: broadcast.read_count, color: 'bg-blue-500' },
    { label: t('funnelSteps.replied'), value: broadcast.replied_count, color: 'bg-indigo-500' },
  ];
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              >
                {tStatuses(broadcast.status as 'draft' | 'scheduled' | 'sending' | 'sent' | 'failed')}
              </span>
            </div>
            <div className="mt-1 flex items-center gap-3 text-sm text-muted-foreground">
              ...
              <span>-</span>
              <span>
                {t('createdPrefix')} {new Date(broadcast.created_at).toLocaleDateString(locale)}
              </span>
            </div>
          </div>
        </div>
        ...
        {confirmDelete ? (
          <div className="flex items-center gap-2 rounded-md border border-destructive/30 bg-destructive/10 px-3 py-1.5 text-sm">
            ...
            <Button
              ...
              disabled={deleting}
              className="h-7 bg-destructive text-destructive-foreground hover:bg-destructive/90 disabled:opacity-50"
            >
          ...
          <Button
            ...
            title={
              broadcast.status === 'sending'
                ? t('deleteTooltipDisabled')
                : t('deleteTooltipEnabled')
            }
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <StatCard
          ...
          color="bg-red-500/10 text-destructive"
        />
      </div>
      ...
      <div className="rounded-xl border border-border bg-card shadow-card">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border px-4 py-3">
          <h2 className="text-sm font-medium text-foreground">
            {t('recipientsTitle')} ({filteredRecipients.length}
          ...
          <div className="flex items-center gap-2">
            <DropdownMenu>
              ...
              >
                ...
                {statusFilter === 'all'
                  ? t('filterAll')
                  : tRecipientStatuses(statusFilter)}
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-border hover:bg-transparent">
                  <TableHead className="text-muted-foreground">{t('headers.contact')}</TableHead>
                  ...
                  <TableHead className="text-muted-foreground">{t('headers.sent')}</TableHead>
                  <TableHead className="text-muted-foreground">{t('headers.delivered')}</TableHead>
                  <TableHead className="text-muted-foreground">{t('headers.read')}</TableHead>
                  <TableHead className="text-muted-foreground">{t('headers.error')}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                  ...
                  return (
                    <TableRow key={recipient.id} className="border-border">
                      ...
                      <TableCell>
                        ...
                        >
                          {tRecipientStatuses(recipient.status)}
                        </span>
                      </TableCell>
