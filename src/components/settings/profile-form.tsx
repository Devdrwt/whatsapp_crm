...
export function ProfileForm() {
  ...
  const onPickFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!ALLOWED_MIME.has(file.type)) {
      toast.error(t('toasts.unsupportedImage'), {
        description: t('toasts.unsupportedImageHint'),
      });
      return;
    }
    if (file.size > MAX_AVATAR_BYTES) {
      toast.error(t('toasts.imageTooLarge'), {
        description: t('toasts.imageTooLargeHint'),
      });
      return;
    }
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (pendingAvatar) {
        ...
        if (uploadError) {
          throw new Error(t('toasts.uploadFailed', { error: uploadError.message }));
        }
        const {
          data: { publicUrl },
      ...
      const { error: updateError } = await supabase
        ...
        .eq('user_id', user.id);
      if (updateError) {
        throw new Error(t('toasts.saveFailed', { error: updateError.message }));
      }
      ...
      if (trimmedEmail.toLowerCase() !== profile.email.toLowerCase()) {
        ...
        if (emailError) {
          ...
          toast.error(t('toasts.emailChangeFailed', { error: emailError.message }));
          setSaving(false);
      ...
      toast.success(emailSent ? t('toasts.savedEmailPending') : t('toasts.saved'));
    } catch (err) {
  ...
  return (
    <Card className="bg-muted/40 border-border shadow-card">
      <CardHeader>
              ...
              <p className="flex items-start gap-2 rounded-md border border-amber-500/30 bg-amber-500/5 px-3 py-2 text-xs text-amber-600 dark:text-amber-300">
                ...
                <span>
                  {t.rich('emailChangePending', {
                    oldEmail: () => <strong>{profile?.email}</strong>,
