...
function MediaUnavailable({ label }: { label: string }) {
  ...
  return (
    <div className="flex items-center gap-2 rounded-lg bg-muted/40 px-3 py-2 text-xs text-foreground">
      ...
      <span>{t("mediaUnavailable", { label })}</span>
    </div>
  );
}
  ...
  switch (message.content_type) {
    ...
    case "image":
      return (
        <div>
          {message.media_url ? (
            <MediaImage url={message.media_url} alt={t("imageAlt")} />
          ) : (
            <MediaUnavailable label={t("mediaLabels.image")} />
          )}
          {message.content_text && (
      ...
      return (
        <div>
          {message.media_url ? (
            ...
            <MediaUnavailable label={t("mediaLabels.video")} />
          )}
          {message.content_text && (
            <p className="mt-1 whitespace-pre-wrap break-words text-sm">
              {message.content_text}
      ...
      return (
        <div>
          {message.media_url ? (
            ...
            <MediaUnavailable label={t("mediaLabels.audio")} />
          )}
        </div>
      );
    ...
    case "document":
      if (!message.media_url) {
        return <MediaUnavailable label={message.content_text || t("mediaLabels.document")} />;
      }
      return (
        <a
        ...
        >
          ...
          <span className="truncate">
            {message.content_text || t("mediaLabels.document")}
          </span>
        </a>
      );
    ...
    default:
      return (
        <p className="whitespace-pre-wrap break-words text-sm">
          {message.content_text || t("unsupportedFallback")}
        </p>
      );
  }
}
